...
def _resolve_input_paths(config_hash: str, input_dir: str) -> tuple[Path, Path]:
    base_dir = Path(input_dir)
    artifact_path = base_dir / f"rho_history_{config_hash}.h5"
    config_path = base_dir / f"config_{config_hash}.json"
    return artifact_path, config_path

def validate_run(config_hash: str, input_dir: str):
    print(f"[Validator {config_hash[:8]}] Starting V13 Certified Analysis...")

    # Path Resolution
    prov_dir = Path(settings.PROVENANCE_DIR)
    artifact_path, config_path = _resolve_input_paths(config_hash, input_dir)
    output_path = prov_dir / f"provenance_{config_hash}.json"

    # 1. Refusal Scaffolding: Artifact Existence
    if not artifact_path.exists():
        print(f"[Validator] FAIL: Artifact {artifact_path} not found.")
        _write_failure(output_path, config_hash, settings.SENTINEL_ARTIFACT_MISSING, "ArtifactMissing")
        return
    ...
    try:
        ...
        provenance = {
            ...
            "sentinel_code": settings.SENTINEL_SUCCESS,
        ...
        _write_failure(output_path, config_hash, settings.SENTINEL_SCIENTIFIC_FAILURE, str(e))

def _write_failure(path, config_hash, code, msg):
    data = {
        settings.HASH_KEY: config_hash,
        "metrics": {
...
if __name__ == "__main__":
    ...
    validate_run(args.config_hash, args.input_dir)
